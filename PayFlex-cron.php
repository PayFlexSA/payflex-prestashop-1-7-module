...
if ($payflex->active) {
    ...
    $sql = '
    SELECT c.id_cart, c.id_lang, cu.id_customer, c.id_shop, cu.firstname, cu.lastname, cu.email,pf.*, c.date_add, c.secure_key
    ...
    if(!empty($carts)){
        foreach ($carts as $cartOrder) {
            // Only process orders older than 30 minutes
            if (time() - strtotime($cartOrder['date_add']) < 1800) {
                $pending++;
                continue;
            }
        ...
        if($pending>0){
            echo "<p>Pending Orders: ".$pending." <small>(Orders that were created too recently are intentionally excluded)</small></p>";
        }
        echo "<p>Total Orders Fetched: ".$total."</p>";
    }else{
        echo "<p>No Orders Found</p>";
    }

    die('Payflex CRON Completed');
}
